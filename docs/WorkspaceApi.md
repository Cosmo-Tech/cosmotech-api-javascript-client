...
> [WorkspaceUser] addUsersToOrganizationWorkspace(organizationId, workspaceId, workspaceUser)
...
apiInstance.addUsersToOrganizationWorkspace(organizationId, workspaceId, workspaceUser, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
